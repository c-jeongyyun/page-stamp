...
- [ ] **메시지 타입 인터페이스 정의** (`src/types.ts` 신규)
  - UI → code.ts 메시지: `apply`, `refresh`, `removeAll`, `getComponents`, `getSelection`
  - code.ts → UI 메시지: `selectionUpdate`, `componentList`, `done`, `error`
  - 공유 설정 타입: `PluginSettings` (컴포넌트 ID, 텍스트 레이어명, 포지셔닝 모드, x/y, 번호 포맷, 시작 번호)
...
- [ ] `figma.on('selectionchange')` 핸들러 등록
- [ ] 선택된 노드 중 `FrameNode`만 필터링
- [ ] 선택 변경 시 UI에 `selectionUpdate` 메시지 전송 (프레임 수, 이름 목록)
...
- [ ] `sortByTL(frames: FrameNode[])` 함수 구현
  ...
  - 그 외에는 y 오름차순
- [ ] 정렬 결과를 `selectionUpdate` 메시지에 포함하여 UI에 전달

### 1-3. 파일 내 컴포넌트 목록 조회

- [ ] `figma.root.findAllWithCriteria({ types: ['COMPONENT'] })`로 전체 컴포넌트 조회
- [ ] `{ id, name }` 목록을 UI에 `componentList` 메시지로 전송
...
- [ ] `refreshPageNumbers(frames, settings)` 함수 구현 (spec 6.5 참고)
...
- [ ] `removeAllPageNumbers(frames)` 함수 구현
...
- [ ] 플러그인 시작 시 `getComponents`, `getSelection` 메시지 발송
- [ ] `onmessage` 핸들러 등록 (code.ts 응답 수신)
- [ ] 전역 상태 정의: 선택 프레임 수, 컴포넌트 목록, 폼 설정값

### 2-2. STEP 1 — 프레임 선택 및 순서 확인

- [ ] 선택된 프레임 수 표시 ("선택된 프레임: N개")
- [ ] 프레임 미선택 시 안내 문구 표시 ("캔버스에서 프레임을 선택하세요")

### 2-3. STEP 2 — 컴포넌트 선택

- [ ] 라디오 버튼: "파일 내 컴포넌트 사용" / "기본 컴포넌트 자동 생성"
- [ ] **파일 내 컴포넌트 사용** 선택 시:
  - 컴포넌트 드롭다운 (`<select>`) 표시 — code.ts에서 받은 목록 렌더링
